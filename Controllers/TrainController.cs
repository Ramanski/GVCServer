...
namespace GVCServer.Controllers
{
    ...
    public class TrainController : ControllerBase
    {
        ...
        [HttpGet("{index}")]
        public async Task<ActionResult<TrainModel>> GetTrainInfo(string index)
        {
            TrainModel trainModel = await _trainRepository.GetTrainModelAsync(index);
        ...
        {
            TrainModel trainModel = JsonSerializer.Deserialize<TrainModel>(consistList.TrainInfo);
            return await _trainRepository.AddTrainAsync(trainModel, station);
        }
